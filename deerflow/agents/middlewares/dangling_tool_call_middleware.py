...
It additionally normalizes the order of pre-existing ToolMessages so that each
tool result sits directly after the AIMessage that produced its tool_call.
Strict provider validators (OpenAI / Moonshot / Anthropic) require tool_calls
to be immediately followed by their tool responses; messages inserted between
them by other middleware (e.g. loop warnings, summarization) would otherwise
trigger 400 errors.
...
class DanglingToolCallMiddleware(AgentMiddleware[AgentState]):
    ...
    offending AIMessage so the LLM receives a well-formed conversation. Also
    re-orders existing ToolMessages so that they appear adjacent to the AIMessage
    that produced them.
    """
    ...
    def _build_patched_messages(self, messages: list) -> list | None:
        ...
        while preserving already-valid transcripts unchanged. Returns ``None``
        if no patching or reordering was needed.
        """
        ...
        for msg in messages:
            if isinstance(msg, ToolMessage) and msg.tool_call_id in tool_call_ids:
                # Skip — this ToolMessage will be re-emitted right after its
                # owning AIMessage below.
                continue
