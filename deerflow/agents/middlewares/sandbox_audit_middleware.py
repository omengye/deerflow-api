...
_DEV_TCP_PATTERN = re.compile(
    r"/dev/tcp/(?P<host>localhost|127(?:\.\d{1,3}){3}|172\.17\.0\.1|host\.docker\.internal)/(?P<port>\d{1,5})"
)
_SAFE_DEV_TCP_PROBE_PATTERN = re.compile(
    r"^(?:timeout\s+\d+(?:\.\d+)?\s+)?(?:bash|sh)\s+-c\s+"
    r"(?P<quote>['\"])echo\s*>/dev/tcp/"
    r"(?P<host>localhost|127(?:\.\d{1,3}){3}|172\.17\.0\.1|host\.docker\.internal)/"
    r"(?P<port>\d{1,5})(?P=quote)"
    r"(?:\s+\d?>&\d+)?"
    r"(?:\s*(?:&&|\|\|)\s*echo\s+['\"]?[^;&|<>`$()]*['\"]?)*$"
)
...
def _is_safe_dev_tcp_probe(command: str) -> bool:
    """Return True for the daemon reachability probe generated by host tools.

    Bash's /dev/tcp can bypass normal network tooling, so general usage remains
    high risk.  The allowed shape is intentionally narrow: a timeout-wrapped
    shell that redirects an empty echo to the local/Docker-host daemon port,
    optionally followed by constant echo status messages.
    """
    normalized = " ".join(_strip_full_line_comments(command).split())
    if "/dev/tcp/" not in normalized:
        return False
    matches = list(_DEV_TCP_PATTERN.finditer(normalized))
    if not matches or len(matches) != normalized.count("/dev/tcp/"):
        return False
    for match in matches:
        port = int(match.group("port"))
        if port not in _ALLOWED_DEV_TCP_PROBE_PORTS:
            return False
