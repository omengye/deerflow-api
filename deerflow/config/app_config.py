...
class AppConfig(BaseModel):
    ...
    def from_file(cls, config_path: str | None = None) -> Self:
        ...
        # Load extensions config separately (it's in a different file). Keep
        # honoring the legacy skills.extensions_file value when present.
        extensions_config_path = None
        skills_config = config_data.get("skills")
        if isinstance(skills_config, dict) and skills_config.get("extensions_file"):
            raw_extensions_path = Path(str(skills_config["extensions_file"]))
            if not raw_extensions_path.is_absolute():
                raw_extensions_path = resolved_path.parent / raw_extensions_path
            if raw_extensions_path.exists():
                extensions_config_path = str(raw_extensions_path)

        extensions_config = ExtensionsConfig.from_file(extensions_config_path)
