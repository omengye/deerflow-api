"""Fail-closed ZIP construction for artifacts presented by one run."""
...
_DEFAULT_RESERVED_DIR_NAMES = frozenset({".browser-frames", ".tool-results"})
...
class ArtifactArchiveError(ValueError):
    """An archive cannot be built without violating its safety contract."""
...
def _reject() -> ArtifactArchiveError:
    return ArtifactArchiveError(
        "The files listed by this run are not available for archive download"
    )
...
def _is_link_like(path: Path, metadata: os.stat_result) -> bool:
    ...
    return stat.S_ISLNK(metadata.st_mode) or bool(is_junction and is_junction())
...
def _member(
    ...
    if any(
        ...
        or part.endswith((" ", "."))
        or part.split(".", 1)[0].rstrip().casefold() in _WINDOWS_DEVICE_NAMES
        for part in parts
    ):
        raise _reject()
    if any(
        any(
            ...
            and char not in _ALLOWED_FORMAT_CHARS
            for char in part
        )
        for part in parts
    ):
        raise _reject()
    if any(
        part.casefold() in reserved or part.casefold().startswith(_EDIT_TEMP_PREFIX)
        for part in parts
    ):
        raise _reject()
...
def _copy_member(
    ...
    try:
        ...
        if (
            ...
            or identity != (member.initial.st_dev, member.initial.st_ino)
        ):
            raise _reject()
        if before.st_size > MAX_FILE_BYTES:
            raise _too_large(
                f"Each archived artifact must be at most {MAX_FILE_BYTES} bytes"
            )
        if before.st_size > remaining_total_bytes:
            raise _too_large(
                f"Archived artifacts must total at most {MAX_TOTAL_BYTES} bytes"
            )
        ...
        if (
            ...
            raise _reject()
        if (
            _hash_descriptor(descriptor, before.st_size, deadline)
            != copied_digest.digest()
        ):
            raise _reject()
        after_verification = os.fstat(descriptor)
        if (
            after_verification.st_nlink != 1
            or (after_verification.st_dev, after_verification.st_ino) != identity
            or (after_verification.st_size, after_verification.st_mtime_ns)
            != (before.st_size, before.st_mtime_ns)
        ):
            raise _reject()
        try:
            for component, device, inode in member.components:
                ...
                if _is_link_like(component, current) or (
                    current.st_dev,
                    current.st_ino,
                ) != (device, inode):
                    raise _reject()
...
def build_artifact_archive(
    ...
    *,
    ...
    """Build an uncompressed archive from a validated, run-scoped allowlist."""
    ...
    try:
        ...
        if (
            _is_link_like(user_data_dir, user_data_metadata)
            or not stat.S_ISDIR(user_data_metadata.st_mode)
            or _is_link_like(outputs_dir, outputs_metadata)
            or not stat.S_ISDIR(outputs_metadata.st_mode)
        ):
            raise _reject()
    ...
    reserved = frozenset(
        name.casefold()
        for name in {
            *_DEFAULT_RESERVED_DIR_NAMES,
            *extra_reserved_dir_names,
        }
        if isinstance(name, str) and name
    )
    members = [
        _member(root, path, reserved, deadline, root_components) for path in paths
    ]
    collision_keys = [
        unicodedata.normalize("NFC", member.entry).casefold() for member in members
    ]
    ...
    _check_deadline(deadline)
    # The open file is intentionally returned to the StreamingResponse owner.
    output = tempfile.TemporaryFile("w+b")  # noqa: SIM115
    try:
